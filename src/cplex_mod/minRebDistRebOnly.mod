...
dvar int+ slack[region];

float SLACK_PENALTY = 10000;

minimize(sum(e in edge) (rebFlow[e] * time[e]) + SLACK_PENALTY * sum(i in region) (slack[i]));

subject to
{
  forall(i in region)
    {
    sum(e in edge: e.i==i && e.i!=e.j) (rebFlow[<e.j, e.i>] - rebFlow[<e.i, e.j>]) + slack[i] >= desiredVehicles[i] - vehicles[i];
